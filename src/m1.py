...
# -----------------------------------------------------------------------------
# DONE2: 4. Follow the INSTRUCTIONS AT THE TOP OF THIS MODULE
#          to correct the mistake(s) in the following function.
# -----------------------------------------------------------------------------
...
def broken_7(n, point, length, distance_between_lines, window):
    ...
    for _ in range(n):
        ...
        line.attach_to(window)
        left = rg.Point(left.x, left.y + distance)
        right = rg.Point(right.x, right.y + distance)
        window.render(0.5)
